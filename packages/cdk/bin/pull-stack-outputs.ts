...
async function pullStackOutputs({ region, stackName }: { region: string; stackName: string }): Promise<any[]> {
  const client = new CloudFormationClient(getAwsClientConfig())
...
async function main() {
  try {
    ...
    const stackNames = [
      `Tasks-${environment}-WebApp`,
      `Tasks-${environment}-Database`,
      `Tasks-${environment}-Auth`,
      // `Tasks-${environment}-Generator`,
      `Tasks-${environment}-Api`,
      `Tasks-${environment}-Monitoring`,
    ]
