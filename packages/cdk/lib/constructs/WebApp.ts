...
import { getMissingEnvVars, getMissingEnvVarsMessage } from '@repo/common'
...
export default class WebApp extends Construct {
  ...
  packageWebApp() {
    ...
    const envName = getEnvironmentName(this.node)
    const uiPackagePath = path.resolve(import.meta.dirname, '../../../../apps/ui')
    ...
    if (missingEnvVars.length) {
      ...
      console.info(
        `Skipping web app deploy due to missing environment variables. If you're missing the apps/ui/.env/.env.${envName} file, try running \`npm run copy-outputs-to-dotenv:dev\` to copy values from cdk-outputs.${envName} to the .env file. If the cdk-outputs file also doesn't exist, first run \`npm run pull-stack-outputs:${envName}\`. Then, run \`npm run deploy:${envName}\` again to deploy the web app.`,
      )
      return
    }
