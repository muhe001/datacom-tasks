...
export default function UnauthenticatedPage({ children, pageTitle, includeLayout = true, includeLogoContentWrapper = true }) {
  const router = useRouter()
  const currentUserQuery = useCurrentUserQuery({ redirectOnNotAuth: false })
  useEffect(() => {
    if (currentUserQuery.data) {
      const searchParams = new URLSearchParams(window.location.search)
      const redirect = searchParams.get('redirect') || '/task-items'
                ...
                {includeLogoContentWrapper ?
                  <div>
                    <div style={{ textAlign: 'center', padding: '1rem' }}>
                      <Image className="logo" src="/logo.png" alt="Logo" preview={false} style={{ maxHeight: '100px', maxWidth: '100%' }} />
                    </div>
                    {children}
                  </div>
                : children}
